...
interface ForageableDao {
    ...
    fun getAllForageables(id: Int): Flow<Forageable>

    // TOD: implement a method to insert a Forageable into the database
    //  (use OnConflictStrategy.REPLACE)
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(forageable: Forageable)

    // TOD: implement a method to update a Forageable that is already in the database
    @Update
    suspend fun update(forageable: Forageable)

    // TOD: implement a method to delete a Forageable from the database.
    @Delete
